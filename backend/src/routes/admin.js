...
/** Clear draw results (test runs / redo). side: sellers | buyers | all */
router.post('/contests/:id/reset-draw', async (req, res) => {
  try {
    const side = req.body?.side === 'sellers' || req.body?.side === 'buyers'
      ? req.body.side
      : 'all';
    const result = await resetContestDraw(pool, req.params.id, side);
    if (!result.ok) {
      const status = result.code === 'NOT_FOUND' ? 404 : 400;
      return res.status(status).json({ error: result.error, code: result.code });
    }
    ...
    console.error(err);
    res.status(500).json({ error: 'Не удалось сбросить розыгрыш' });
  }
});
