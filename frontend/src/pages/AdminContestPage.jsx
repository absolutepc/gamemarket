...
import { Trophy, Dices, Store, ShoppingBag, RefreshCw, Play, RotateCcw } from 'lucide-react';
...
export default function AdminContestPage() {
  ...
  const resetMutation = useMutation({
    mutationFn: (side) =>
      api.post(`/admin/contests/${data.contest.id}/reset-draw`, { side }).then((r) => r.data),
    onSuccess: (res) => {
      toast.success(res.message || 'Розыгрыш сброшен');
      qc.invalidateQueries({ queryKey: ['admin-contest-current'] });
    },
    onError: (err) => toast.error(err.response?.data?.error || 'Не удалось сбросить розыгрыш'),
  });
  ...
  const hasAnyDraw = Boolean(contest?.seller_drawn_at || contest?.buyer_drawn_at);

  const runReset = (side) => {
    const labels = {
      sellers: 'продавцов',
      buyers: 'покупателей',
      all: 'обеих сторон (продавцы и покупатели)',
    };
    if (
      !window.confirm(
        `Сбросить розыгрыш ${labels[side] || side}? Победитель будет очищен, можно будет разыграть заново.`
      )
    ) {
      return;
    }
    resetMutation.mutate(side);
  };
  ...
  return (
    <div className={`${PAGE_WIDTH_CLASS} py-8`}>
      ...
      <p className="text-sm text-dark-400 mb-6 max-w-3xl">
        ...
        После тестового розыгрыша используйте «Сбросить розыгрыш».
      </p>
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {hasAnyDraw ? (
                  <button
                    type="button"
                    className="btn-secondary h-9 px-3 inline-flex items-center gap-1.5 text-sm text-amber-200"
                    onClick={() => runReset('all')}
                    disabled={resetMutation.isPending}
                    title="Сбросить оба розыгрыша"
                  >
                    <RotateCcw size={14} />
                    Сбросить всё
                  </button>
          ...
          <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            <div className="ml-auto flex flex-wrap items-center gap-2">
              {alreadyDrawn ? (
                <button
                  type="button"
                  className="btn-secondary h-10 px-4 inline-flex items-center gap-1.5 text-amber-200"
                  disabled={resetMutation.isPending}
                  onClick={() => runReset(tab)}
                  title="Сбросить розыгрыш текущей вкладки"
                >
                  <RotateCcw size={16} />
                  {resetMutation.isPending ? 'Сброс…' : 'Сбросить розыгрыш'}
                </button>
              ) : null}
              <button
                type="button"
